import { IPosto, IPrenotazione } from "../model/IModels";
const bottoneInvia:Element| null = document.querySelector(".bottoneInvia2");
        ...
        .then(data => {
            ...
            if (test && prenotazione) {
                ...
                postiScelti = Array.from(posti).filter((posto) => {
                    return postiPrenotati.some((postoPrenotato) => 
                        posto.dataset.fila === postoPrenotato.fila &&
...
function generaPosti(file:any, posti:any) {
    const container:any = document.getElementById("postiCinema");
    container.innerHTML = "";
...
document.querySelectorAll(".posto").forEach((posto:any) => {
    posto.addEventListener("click", () => {
        posto.classList.toggle("selected")

        if (posto.classList.contains("selected")){
        const postoPrenotazione: IPosto = {fila: posto.dataset.fila, posto: posto.dataset.posto}
        if (!prenotazione.posti) {
            prenotazione.posti = []; 
        }
        prenotazione.posti?.push(postoPrenotazione)
        ...
        console.log(prenotazione)
            ...
            if (prenotazione.posti) {
                prenotazione.posti = prenotazione.posti.filter(
                    (p: IPosto) => p.fila !== fila || p.posto !== numeroPosto
                );
            }
...
const light:any = document.getElementById('light');
